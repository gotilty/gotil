...
func TestIncludes(t *testing.T) {
	...
	for _, test := range testData {
		...
		b, errb := collection.Includes(test.inputValue, test.Includes)
		fmt.Println(a)
		fmt.Println(b)
		if erra == nil {
			if !reflect.DeepEqual(a, b) || errb != nil {
				t.Errorf("collection.Includes does not works expected\ncase")
			}
		}
	}
}
...
func BenchmarkIncludesMathFloor(b *testing.B) {
	testData := getIncludesTestData()
	for n := 0; n < b.N; n++ {
		collection.Includes(testData["float_array"].inputValue, testData["float_array"].Includes)
	}
}
...
func BenchmarkIncludesStringLen(b *testing.B) {
	testData := getIncludesTestData()
	for n := 0; n < b.N; n++ {
		collection.Includes(testData["map"].inputValue, testData["map"].Includes)
	}
}
