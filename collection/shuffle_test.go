...
func TestShuffle(t *testing.T) {
	...
	for key, test := range testData {
		...
		b, errb := collection.ShuffleSeed(test.inputValue, test.seed)
		if erra == nil {
...
func BenchmarkShuffleIntegerSlice(b *testing.B) {
	testData := getShuffleTestData()
	for n := 0; n < b.N; n++ {
		collection.Shuffle(testData["shuffle_numbers"].inputValue)
	}
}
...
func BenchmarkShuffleStructSlice(b *testing.B) {
	testData := getShuffleTestData()
	for n := 0; n < b.N; n++ {
		collection.Shuffle(testData["shuffle_struct"].inputValue)
	}
}
...
func ExampleShuffle() {
	...
	// Input: [-100 -5 30 100]
	newData, _ := collection.ShuffleSeed(data, seed)
	fmt.Println(newData)
	// Output: [-5 100 -100 30]
}
