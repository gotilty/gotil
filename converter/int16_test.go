...
func TestConvertToInt16(t *testing.T) {
	...
	for key, test := range testData {
		...
		b, errb := converter.ToInt16(test.inputValue)
		if erra == nil {
			if a != b || errb != nil {
...
func BenchmarkConvertToInt16String(b *testing.B) {
	testData := getConvertToInt16TestData()
	for n := 0; n < b.N; n++ {
		converter.ToInt16(testData["string"].inputValue)
	}
}
func BenchmarkConvertToInt16Integer(b *testing.B) {
	testData := getConvertToInt16TestData()
	for n := 0; n < b.N; n++ {
		converter.ToInt16(testData["integer"].inputValue)
	}
}
...
func BenchmarkConvertToInt16Struct(b *testing.B) {
	testData := getConvertToInt16TestData()
	for n := 0; n < b.N; n++ {
		converter.ToInt16(testData["struct"].inputValue)
	}
}
func BenchmarkConvertToInt16Uint(b *testing.B) {
	testData := getConvertToInt16TestData()
	for n := 0; n < b.N; n++ {
		converter.ToInt16(testData["uint"].inputValue)
	}
}
