...
func TestConvertToInt(t *testing.T) {
	...
	for key, test := range testData {
		...
		b, errb := converter.ToInt(test.inputValue)
		if erra == nil {
			if a != b || errb != nil {
...
func BenchmarkConvertToIntString(b *testing.B) {
	testData := getConvertToIntTestData()
	for n := 0; n < b.N; n++ {
		converter.ToInt(testData["string"].inputValue)
	}
}
func BenchmarkConvertToIntInteger(b *testing.B) {
	testData := getConvertToIntTestData()
	for n := 0; n < b.N; n++ {
		converter.ToInt(testData["integer"].inputValue)
	}
}
...
func BenchmarkConvertToIntStruct(b *testing.B) {
	testData := getConvertToIntTestData()
	for n := 0; n < b.N; n++ {
		converter.ToInt(testData["struct"].inputValue)
	}
}
func BenchmarkConvertToIntUint(b *testing.B) {
	testData := getConvertToIntTestData()
	for n := 0; n < b.N; n++ {
		converter.ToInt(testData["uint"].inputValue)
	}
}
